...
| Previous stable version | None                                                         |
| Release type            | First stable production release                              |
| Version strategy        | Semantic Versioning                                          |
| Promotion rule          | Carry forward the validated RC.1 safety and command contract |

### Stable feature baseline

#### CI/CD baseline promoted from RC.1

- [x] Carry forward the Python 3.11 and 3.14 CI matrix.
...
- [x] Carry forward recursive, language-aware path-header discovery and validation.
- [x] Carry forward preview-first scanning and explicit `--apply` mutation control.
- [x] Carry forward Python, JavaScript, TypeScript, Shell, PHP, HTML, and Markdown strategies.
- [x] Carry forward deterministic repository-relative paths and configurable ignore rules.
- [x] Carry forward safe initialization and namespaced `.config/path_header_scanner/config.toml` configuration.
- [x] Carry forward authoritative dry-run behavior over CLI and configured apply modes.
- [x] Carry forward Rich help, progress, summaries, logging, actionable failures, and debug diagnostics.
- [x] Carry forward the modular architecture, tests, documentation, Make, Docker, Compose, packaging, and CI/CD workflows.

### Stable-release finalization

- [ ] Incorporate only release-blocking fixes and documentation corrections identified during RC validation.
- [ ] Remove temporary development artifacts that are not part of the supported product.
- [ ] Review package metadata, generated configuration, command help, CI/CD, docs, and container destinations.
- [ ] Confirm preview mode remains the default and `--apply` is the only supported write opt-in.
- [ ] Confirm dry-run stops all target-project mutations.
- [ ] Preserve the completed development history and remaining future work in project tracking documentation.

### Stable validation checklist

- [ ] Run the complete approved tests and coverage suite.
- [ ] Run Ruff, Black, pre-commit, packaging, and documentation validation.
- [ ] Validate initialization and scan help locally and in production containers.
- [ ] Validate preview, apply, and dry-run in disposable multi-language repositories.
- [ ] Validate Make and Docker Compose development and production workflows.
- [ ] Review generated headers for path correctness and language-appropriate syntax.
- [ ] Commit and create the `v1.0.0` tag only with explicit release approval.
- [ ] Verify release artifacts, packages, container images, and documentation after publication.

### Stable support boundaries

- [x] Scan targets are processed one resolved target at a time.
- [x] Unsupported file types are reported or skipped rather than mutated.
- [x] Diagnostic logging may write during dry-run, but target-project files may not.
- [x] Generated configuration is project-owned after initialization.
- [x] Python 3.11+ is the package compatibility floor; Python 3.14 is the standard development and container runtime.

### Future work

- [ ] Evaluate multiple-target scans without weakening path determinism or write safety.
- [ ] Improve progress detail for very large targets when measurements justify it.
- [ ] Add new language strategies through the existing extension boundary and full preview/apply tests.
- [ ] Review and intentionally refresh pre-commit hook versions and Python targets.

### Notes

- Stable 1.0 should be a promotion of reviewed RC.1 behavior, not an unreviewed
  expansion of the command or mutation model.
- Unchecked release operations remain pending until explicitly authorized.
...
### Path Header Scanner TODO Tracking History — v1.0.0.md
...
- [x] Complete Path Header Scanner suite: 127 tests passed with 84% overall coverage.
